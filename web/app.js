...
import init, { WasmGame } from './pkg/worm.js?v=59';
import { Sfx } from './audio.js?v=59';
...
function loop(now) {
  ...
  let delay = touchPace(Number(game.frame_delay_ms()));
  let steps = 0;
  ...
  while (acc >= delay && steps < 2) {
    ...
    acc -= delay;
    steps++;
    delay = touchPace(Number(game.frame_delay_ms()));
...
function hud(s) {
  ...
  // screen ~6s so the causality lands.
  {
    ...
    if (el) {
      // frame resets at round boundaries: a receipt from a previous
      // round (frame > brain.frame) is stale, not ultra-fresh.
      const fresh = ex && ex.frame <= state.brain.frame
        && state.brain.frame - ex.frame < 180;
